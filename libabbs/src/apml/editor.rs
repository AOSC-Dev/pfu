...
impl<'a> ApmlEditor<'a> {
    ...
    pub fn lst_variables(&self) -> impl Iterator<Item = &lst::VariableDefinition<'a>> {
    ...
    pub fn ast_variables(&self) -> ast::EmitResult<Vec<ast::VariableDefinition<'a>>> {
        self.lst_variables()
    ...
    pub fn replace_var_ast(&mut self, name: &'a str, value: &ast::VariableValue<'a>) {
        let definition = lst::VariableDefinition {
            name: name.into(),
            op: lst::VariableOp::Assignment,
            value: value.lower(),
        ...
        if matches!(tokens.get(index - 2), Some(lst::Token::Comment(_))) {
                ...
                .any(|token| matches!(token, lst::Token::Variable(_)))
            {
                ...
                while matches!(tokens.get(start - 1), Some(lst::Token::Newline))
                    && matches!(tokens.get(start - 2), Some(lst::Token::Comment(_)))
                    && matches!(tokens.get(start - 3), Some(lst::Token::Newline))
                {
                    start -= 2;
                }
            }
...
mod test {
    ...
    fn test_append_variable() {
        ...
        editor.append_var_ast("c", &ast::VariableValue::String("a".into()), None);
        assert_eq!(editor.unwrap().to_string(), "a=b\nb=c\nc=\"a\"\n");
        let mut editor = ApmlEditor::wrap(ApmlLst::parse("a=b\nb=c").unwrap());
        editor.append_var_ast("c", &ast::VariableValue::String("a".into()), Some("a"));
        assert_eq!(editor.unwrap().to_string(), "a=b\nc=\"a\"\nb=c");
        let mut editor = ApmlEditor::wrap(ApmlLst::parse("a=b\nb=c").unwrap());
        editor.append_var_ast("c", &ast::VariableValue::String("a".into()), Some("b"));
        assert_eq!(editor.unwrap().to_string(), "a=b\nb=c\nc=\"a\"\n");
        let mut editor = ApmlEditor::wrap(ApmlLst::parse("a=b\nb=c").unwrap());
        editor.append_var_ast("c", &ast::VariableValue::String("a".into()), Some("eee"));
        assert_eq!(editor.unwrap().to_string(), "a=b\nb=c\nc=\"a\"\n");
    }
    ...
    fn test_remove_var() {
        ...
        assert_eq!(editor.unwrap().to_string(), "a=b\n\nc=\"$1\"");
        let mut editor =
            ApmlEditor::wrap(ApmlLst::parse("a=b # a\n# b\n# c\nb=c\n\n# a\nc=\"$1\"").unwrap());
        editor.remove_var(editor.find_var("b").unwrap().0);
